package ks.action
{
	...
	final public class TweenFactory
	{
		...
		public static function newTween(targets:*, properties:*, 
			...
			for each (var target:Object in targetsArray) {
				for each (var property:String in propertiesArray) {
					tweenTargets.push(new PropertyTweenTarget(target, property, startValue, endValue));
				}
			}
		...
		public static function newTimelineTween(target:MovieClip,
			...
			var timelineController:TimelineTarget = new TimelineTarget(target, startFrame, endFrame);
			if (duration == TimelineTarget.AUTO) {
				duration = timelineController.getNaturalDuration(TimelineTarget.AUTO);
			}
		...
		public static function newPositionTween(target:DisplayObject, 
			...
			var x:ITweenTarget = new PropertyTweenTarget(target, "x", xFrom, xTo);
			var y:ITweenTarget = new PropertyTweenTarget(target, "y", yFrom, yTo);
...
//		}
		...
		public static function newScaleTween(target:DisplayObject, 
			...
			var xScale:ITweenTarget = new PropertyTweenTarget(target, "scaleX", startValue, endValue);
			var yScale:ITweenTarget = new PropertyTweenTarget(target, "scaleY", startValue, endValue);
		...
		public static function newScaleTweenAdvanced(target:DisplayObject, 
			...
			var xScale:ITweenTarget = new PropertyTweenTarget(target, "scaleX", startValueX, endValueX);
			var yScale:ITweenTarget = new PropertyTweenTarget(target, "scaleY", startValueY, endValueY);
		...
		public static function newFilterTween(target:DisplayObject, 
											  ...
											  duration:* = 0, delay:* = 0, 
											  easingFunction:Function = null, easingMod1:Number = NaN, easingMod2:Number = NaN):KSTween {
			return newTweenWithTargets (new FilterPropertyTweenTarget(target, filter, filterProperty, startValue, endValue), duration, delay, easingFunction, easingMod1, easingMod2);
		}
	}
}